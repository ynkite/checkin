...
#
#   ★ 빗나갔을 때 눈대중으로 옮기지 마라. 회전 때문에 위도를 내리면 화면에서
#     아래로 갈지 위로 갈지 알 수 없다. 성산을 눈대중으로 옮겼더니 119%에서
#     124%로 더 나빠졌다. 계산해서 옮겨라 —
#
#       지금 fit 으로 화면 한가운데(50,50)에 떨어지는 지점 Q 를 역산하고
#       (a·lon + b·lat = 50-c, d·lon + e·lat = 50-f 를 푼다),
#       bbox 를 (랜드마크 - Q) 만큼 옮기면 랜드마크가 Q 자리로 온다.
SCENES = {
    'haeundae':     ((35.1505, 129.1440, 35.1625, 129.1665), 16, 0.74),
    'gwangalli':    ((35.1470, 129.1080, 35.1590, 129.1305), 16, 0.74),
    # 「사람이 몰려서」 갈래의 대체지. 해운대 바로 옆인데 해변이 아니다 —
    # 해변에서 해변으로 옮기면 관광지 간 이동이 아니라 우회로 보인다는 지적을 받았다.
    # 명소를 bbox 한가운데 두고 테두리를 bbox 에 맞춘다(다섯째 칸 True)
    # 메인 화면 확정 동선 (docs-private/메인동선_실측.json).
    # 좌표는 카카오 로컬에서 조회한 값이고 도로 경로는 TMAP 실측이다.
    # 명소를 bbox 한가운데 두고 테두리를 bbox 에 맞춘다(다섯째 칸 True)
    'marine':       ((35.14865, 129.13152, 35.16065, 129.15402), 16, 0.74, 26.0, True),  # 해운대 영화의거리 35.15465,129.14277
    'sparland':     ((35.16282, 129.11827, 35.17482, 129.14077), 16, 0.74, 26.0, True),  # 스파랜드 센텀 35.16882,129.12952
    'sydneysalt':   ((35.15100, 129.13683, 35.16300, 129.15933), 16, 0.74, 26.0, True),  # 시드니앤솔트 35.15700,129.14808
    'silla':        ((35.15380, 129.14749, 35.16580, 129.16999), 16, 0.74, 26.0, True),  # 신라스테이 해운대 35.15980,129.15874
    'dongbaek':     ((35.14791, 129.14087, 35.15991, 129.16337), 16, 0.74, 26.0, True),  # 동백섬 35.15391,129.15212
    ...
    'jeonju':       ((35.8091, 127.1399, 35.8211, 127.1624), 16, 0.74, 110), # 한옥마을 35.8148,127.1526 · 경기전 35.8154,127.1497 (한옥이 촘촘해 1.9MB 였다)

    # ── 전국 — 시도마다 한 곳씩. 없는 지역에서 「모형이 없습니다」가 뜨지 않게.
    # bbox 는 명소를 한가운데 두고 위도 0.012 · 경도 0.0225 (기존 장면과 같은 크기).
    # 다섯째 칸 True 는 테두리를 bbox 에 맞추라는 뜻 — 네모 가운데가 화면 가운데로 간다.
    # 눈대중으로 옮길 필요가 없다 (위 frame_bbox 설명 참고).
    'incheon_china':   ((37.4685, 126.60675, 37.4805, 126.62925), 16, 0.74, 26.0, True),  # 인천 차이나타운·월미도 37.4745,126.618
    'daegu_dongseong': ((35.8628, 128.58375, 35.8748, 128.60625), 16, 0.74, 26.0, True),  # 대구 동성로 35.8688,128.595
    'daejeon_sungsim': ((36.322, 127.41575, 36.334, 127.43825), 16, 0.74, 26.0, True),  # 대전 은행동 36.328,127.427
    'gwangju_acc':     ((35.1404, 126.90875, 35.1524, 126.93125), 16, 0.74, 26.0, True),  # 광주 국립아시아문화전당 35.1464,126.92
    'ulsan_taehwa':    ((35.538, 129.30475, 35.55, 129.32725), 16, 0.74, 26.0, True),  # 울산 태화강 국가정원 35.544,129.316
    'sejong_lake':     ((36.499, 127.24875, 36.511, 127.27125), 16, 0.74, 26.0, True),  # 세종 호수공원 36.505,127.26
    'suwon_hwaseong':  ((37.281, 127.00275, 37.293, 127.02525), 16, 0.74, 26.0, True),  # 수원 화성 37.287,127.014
    'cheongju_suam':   ((36.633, 127.47875, 36.645, 127.50125), 16, 0.74, 26.0, True),  # 청주 수암골 36.639,127.49
    'gongju_fortress': ((36.461, 127.11275, 36.473, 127.13525), 16, 0.74, 26.0, True),  # 공주 공산성 36.467,127.124
    'suncheon_bay':    ((34.879, 127.49775, 34.891, 127.52025), 16, 0.74, 26.0, True),  # 순천만 34.885,127.509
    'pohang_yeongil':  ((36.05, 129.36875, 36.062, 129.39125), 16, 0.74, 26.0, True),  # 포항 영일대 36.056,129.38
    'changwon_jinhae': ((35.144, 128.65875, 35.156, 128.68125), 16, 0.74, 26.0, True),  # 창원 진해 여좌천 35.15,128.67
    'sokcho_abai':     ((38.201, 128.58275, 38.213, 128.60525), 16, 0.74, 26.0, True),  # 속초 아바이마을 38.207,128.594
    'gunsan_modern':   ((35.98, 126.70175, 35.992, 126.72425), 16, 0.74, 26.0, True),  # 군산 근대역사거리 35.986,126.713
}

# ── 전국 — 시군구 251곳 ─────────────────────────────────────
#
# 손으로 적지 않는다. coords.json 에 시군구마다 「관광공사 거점 관광지 1위」
# 좌표가 들어 있고(없으면 「○○청」), 그 자리를 한가운데 두고 bbox 를 만든다.
#
# 시청이 아니라 거점을 쓰는 이유 — 그 지역에서 방문이 실제로 몰리는 자리라
# 우리가 짜는 동선도 그 근처에 생긴다. 시청 앞은 대개 아무 일도 안 일어난다.
#
# 이름은 sig_<법정코드> 가 아니라 sig_<번호> 로 둔다. 파일 이름에 한글을 넣으면
# 서버·배포·깃에서 인코딩이 한 번씩 어긋난다. 어느 지역인지는 SIGUNGU_NAME 이 안다.
SIGUNGU_NAME = {}
def _load_sigungu():
    import json as _json, os as _os
    here = _os.path.dirname(_os.path.abspath(__file__))
    f = _os.path.join(here, 'coords.json')
    if not _os.path.exists(f):
        return
    for i, a in enumerate(_json.load(open(f, encoding='utf-8'))):
        if not a.get('lat'):
            continue
        key = 'sig_%03d' % i
        la, lo = a['lat'], a['lng']
        SCENES[key] = ((round(la - .006, 5), round(lo - .01125, 5),
                        round(la + .006, 5), round(lo + .01125, 5)), 16, 0.74, 26.0, True)
        SIGUNGU_NAME[key] = a['key']
_load_sigungu()
