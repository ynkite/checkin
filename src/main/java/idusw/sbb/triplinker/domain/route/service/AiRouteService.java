...
public class AiRouteService {
    ...
    public String generateAiRoute(Long tripId) {
        ...
        String prompt = String.format("""
            ...
            1. 장소명(name) 엄격 통제 및 일치: 당신은 정보가 부족할 때 그럴듯한 장소를 지어내는(Hallucination) 치명적인 버그가 있습니다. 이를 방지하기 위해 "합천스카이밸리", "OO먹자골목", "OO서비스"처럼 모호하거나 합성된 가짜 이름을 절대 금지합니다. 또한 장소명 뒤에 '체크인', '방문', '식사' 같은 임의의 서술형 접미사를 붙이지 마십시오. 무조건 검색 포털(카카오맵/네이버 지도)에 공식적으로 등록된 [실존하는 유명 랜드마크, 실제 유명 식당 상호명, 실제 숙박업소명]만 엄선해서 정확히 기입하세요. (예: "제주신라호텔", "성산일출봉")
                - 카카오맵 API 검색 가이드 필수 준수: 카카오 지도(KakaoMap)에서 검색했을 때 정확히 단 한 개의 결과만 매칭될 수 있는 [실제 전체 상호명] 또는 **[공식 행정 구역 명칭]**으로만 입력하세요.
                - 모호한 명칭 및 약어 금지: "홍대 맛집", "강릉 카페", "부산 횟집" 같은 카테고리성 명칭이나 키워드 검색 조합은 절대 금지합니다. 반드시 "연돈", "테라로사 커피공장 강릉본점", "해운대해수욕장"처럼 고유 식별이 가능한 구체적인 명칭이어야 합니다.
                - 폐업 및 가상 장소 필터링: 2026년 현재 기준으로 이미 폐업했거나, 아직 개장하지 않은 곳, 혹은 인터넷에 검색되지 않는 가상의 장소는 리스트에서 원천 배제하십시오. 확실하게 대중적으로 검증된 실존 장소만 엄선하세요.
    ...
    public String replaceAiRoutePlaces(Long tripId, java.util.List<java.util.Map<String, String>> requests) {
        ...
        String prompt = String.format("""
            ...
            3. 장소명(name) 엄격 통제 및 일치: 당신은 정보가 부족할 때 그럴듯한 장소를 지어내는(Hallucination) 치명적인 버그가 있습니다. 이를 방지하기 위해 "합천스카이밸리", "OO먹자골목", "OO서비스"처럼 모호하거나 합성된 가짜 이름을 절대 금지합니다. 또한 장소명 뒤에 '체크인', '방문', '식사' 같은 임의의 서술형 접미사를 붙이지 마십시오. 무조건 검색 포털(카카오맵/네이버 지도)에 공식적으로 등록된 [실존하는 유명 랜드마크, 실제 유명 식당 상호명, 실제 숙박업소명]만 엄선해서 정확히 기입하세요. (예: "제주신라호텔", "성산일출봉")
                - 카카오맵 API 검색 가이드 필수 준수: 카카오 지도(KakaoMap)에서 검색했을 때 정확히 단 한 개의 결과만 매칭될 수 있는 [실제 전체 상호명] 또는 **[공식 행정 구역 명칭]**으로만 입력하세요.
                - 모호한 명칭 및 약어 금지: "홍대 맛집", "강릉 카페", "부산 횟집" 같은 카테고리성 명칭이나 키워드 검색 조합은 절대 금지합니다. 반드시 "연돈", "테라로사 커피공장 강릉본점", "해운대해수욕장"처럼 고유 식별이 가능한 구체적인 명칭이어야 합니다.
                - 폐업 및 가상 장소 필터링: 2026년 현재 기준으로 이미 폐업했거나, 아직 개장하지 않은 곳, 혹은 인터넷에 검색되지 않는 가상의 장소는 리스트에서 원천 배제하십시오. 확실하게 대중적으로 검증된 실존 장소만 엄선하세요.
