...
public class AiRouteService {
    ...
    public String generateAiRoute(Long tripId) {
        ...
        String prompt = String.format("""
            ...
            [출력 규칙 - 매우 중요]
            'budget'에는 해당 일차 총액(예: ₩182,000), 'stars'에는 별점(예: ★★★★★ 4.8), 'sub'에는 상세 정보(예: 숙소 · ₩180,000)를 규격화하여 넣으세요.
            JSON 내부에 위도(lat)나 경도(lng) 필드는 절대로 포함하지 마십시오.
    ...
    public String replaceAiRoutePlaces(Long tripId, java.util.List<java.util.Map<String, String>> requests) {
        ...
        String prompt = String.format("""
            ...
            2. [여행 기본 정보]의 예산, 인원, 동행자 특성(유아/반려동물 등)을 반드시 고려하여 조건에 완벽하게 부합하는 가장 적합한 곳으로 교체하세요.
            3. 교체되는 새로운 장소는 카카오맵/네이버 지도에 등록된 실존하는 상호명이어야 합니다. (예: "애견호텔" (X) -> "마이펫 애견호텔 광안점" (O))
            4. 교체된 장소의 금액('sub' 필드)과 해당 일차의 전체 'budget'은 자연스럽게 맞추세요.
            5. 이전과 마찬가지로 위도(lat), 경도(lng) 필드는 생성하지 마십시오.
            6. 마크다운 기호(```json) 없이 오직 순수한 JSON 배열 포맷만 출력하세요.
            """,
                plan.getDestination(),
