...
public class CustomOAuth2UserService extends DefaultOAuth2UserService {
    ...
    public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
        // 구글/카카오에서 보낸 유저 정보 가져오기
        OAuth2User oAuth2User = super.loadUser(userRequest);

        // 어디서 왔는지(google, kakao) 확인
        String provider = userRequest.getClientRegistration().getRegistrationId();

        // 소셜 고유 ID 추출 (구글은 'sub', 카카오는 'id' 라는 이름으로 줍니다)
        String providerId = extractProviderId(provider, oAuth2User.getAttributes());

        // OAuthAccount에 있는 사람인지 확인하고, 없으면 자동 회원가입
        ...
        // oauthAccountRepository.save(oauthAccount); ← @Transactional이므로 Dirty Checking으로 자동 반영됨
        String currentAccessToken = userRequest.getAccessToken().getTokenValue();
        oauthAccount.updateAccessToken(currentAccessToken);

        // User 테이블에서 이 사람의 회원 정보 가져오기
        User user = userRepository.findById(oauthAccount.getUserId())
                .orElseThrow(() -> new IllegalStateException("소셜 계정과 연결된 유저를 찾을 수 없습니다."));

        // CustomUserDetails를 만들어서 시큐리티에 전달
    ...
    private OAuthAccount registerNewOAuthUser(String provider, String providerId, Map<String, Object> attributes) {
        //User 테이블에 넣을 임시 아이디 생성 (예: kakao_123456789)
        String dummyUsername = provider + "_" + providerId;

        //소셜 응답에서 이메일·이름 추출 (provider마다 구조가 다름)
        String email = extractEmail(provider, providerId, attributes);
        String name  = extractName(provider, attributes);

        if (!email.endsWith("@oauth.noemail") && userRepository.existsByEmail(email)) {
            throw new OAuth2AuthenticationException(
                    new OAuth2Error("email_already_exists", "이미 가입된 이메일입니다.", null)
            );
        }

        // User 테이블에 회원가입(소셜은 비밀번호가 없으니 UUID로 쓰레기값 넣기)
